...
class RingSegment(object):
    ...
    def arcFixes(self, lw=0):
        ...
        afix = lambda r: math.degrees(math.asin(float(lw) / r)) if r else 0
...
# NOTE: can be optimized -- as we already knew which dots influence which side
        ...
        r = self.limitAt(0, pts); t = self.limitAt(1, pts)
        l = self.limitAt(2, pts); b = self.limitAt(3, pts)
        pts = lrotate(self.points_ra(), -int((self.a+self.da) / 90))
        r = max(r, self.limitAt(0, pts)); t = max(t, self.limitAt(1, pts))
        l = min(l, self.limitAt(2, pts)); b = min(b, self.limitAt(3, pts))
